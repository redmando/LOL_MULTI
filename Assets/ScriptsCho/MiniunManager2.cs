...
public class MiniunManager2 : MonoBehaviour
{
    Animator ani;
    NavMeshAgent nav;
    ...
    public Transform tsMissle; //미사일 발사 위치(지팡이 끝)
    ...
    public void AttackMissle()
    {
        //함수가 호출되면 미사일 발사
        GameObject obj = Instantiate(missle, tsMissle.position, tsMissle.rotation);
    ...
    {
        //함수가 호출되면 미사일 발사
        GameObject obj = Instantiate(Arrow, tsMissle.position, tsMissle.rotation);
        obj.GetComponent<ArrowMoveCHO>().TargetTr = objTarget.transform;
