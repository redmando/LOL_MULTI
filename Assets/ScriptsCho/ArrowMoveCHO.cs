...
public class ArrowMoveCHO : MonoBehaviour
{
    ...
    {
        yield return new WaitForSeconds(15.0f);      //  지연시간.
        Destroy(gameObject);
    }
    ...
    void DiffusionMissileMoveOperation()
    {
        ...
        if (waitTime < 1.5f)
        {
            ...
            transform.Translate(-Tr.right * speed, Space.World);
        ...
        else
        {
            ...
            if (TargetTr.gameObject != null) 
                Tr.position = Vector3.LerpUnclamped(Tr.position, TargetTr.position, t);
            //Tr.position = Vector3.Lerp(Tr.position, TargetTr.position, Time.deltaTime * fSpeed);
        }
        ...
        //Tr.rotation = Quaternion.Slerp(Tr.rotation, Tr.rotation,Time.deltaTime*2.0f);
